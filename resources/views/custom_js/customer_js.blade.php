   ...
   $(document).ready(function() {
       ...
       $('.add_customer').off().on('submit', function(e){
           ...
           if(id!='')
           {
               ...
               if(id_no=="" )
               {
                   show_notification('error','<?php echo trans('messages.add_id_no_lang',[],session('locale')); ?>'); return false;
               }
               if(license_no=="" )
               {
                   show_notification('error','<?php echo trans('messages.add_license_no_lang',[],session('locale')); ?>'); return false;
               }
               $('#global-loader').show();
               before_submit();
       ...
       $.ajax ({
           ...
           success: function(fetch) {
               ...
               if(fetch!=""){
                   ...
                   $(".id_no").val(fetch.id_no);
                   $(".id_expiry").val(fetch.id_expiry);
                   $(".license_no").val(fetch.license_no);
                   $(".license_expiry").val(fetch.license_expiry);
                   if (fetch.doc_type == 1) {
